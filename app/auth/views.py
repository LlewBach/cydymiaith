...
def send_confirmation(email):
    ...
    Args:
        email (str, optional): The email address to which the confirmation email
        will be sent.

    Returns:
        Response: A redirect response to either the login page or the home page,
        based on the input.
    """
    if email:
...
def reset_password(token):
    ...
    This function handles both GET and POST requests:
      ...
      - For POST requests, it processes the new password submission:
        - If the token is valid and the new password meets criteria, updates the
        user's password.
...
def forgot_password():
    ...
    This endpoint functions differently based on the request method:
      ...
      email address is provided, it redirects the user to the route that sends a
...
def reg_confirmation():
    ...
    This function operates differently based on the HTTP request method:
      - GET: Renders a form for users to submit their email address to receive a
    ...
    Returns:
        Response:
            - On GET requests: Renders the 'reg_email.html' template where users
...
def register(token):
    ...
    Raises:
        SignatureExpired: If the token has expired, indicating a timeout for the
        registration link.
        BadSignature: If the token is invalid, indicating a potentially tampered
        or incorrect link.
    """
    if token:
        try:
...
def profile(username):
    ...
    provided, or if the specified username does not match any existing user, the
    function defaults to the profile of the currently authenticated user. In the
    case of an incorrect or missing username, a flash message is displayed and 
...
def user_owns_profile_or_admin(f):
    ...
    specified in the route parameter or if the current user has an "Admin" role.
    If neither condition is met, the decorator flashes an error message about 
    unauthorized access and redirects the user to their own profile page.
...
def delete_profile(username):
    ...
    Verifies if the current user is authorized to delete the profile (either the
    profile owner or an Admin). If authorized, it deletes the user's profile, 
    flashes a success message, and redirects accordingly.
...
def view_users():
    ...
    (level, provider, username, email, location) and then renders the users view
    template with the filtered results.
